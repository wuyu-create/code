...
from Initial_data_process.smooth_bbox import get_all_bbox_params
from Initial_data_process.img_utils import get_single_image_crop_demo
...
def inference(image_folder, bboxes=None, joints2d=None, scale=1.0, crop_size=224):
    ## 判断文件是否存在
    image_file_names = [
        osp.join(image_folder, x)
        for x in os.listdir(image_folder)
    ...
    for idx in range(len(image_file_names)):
        img = cv2.cvtColor(cv2.imread(image_file_names[idx]), cv2.COLOR_BGR2RGB)
        bbox = bboxes[idx]
        j2d = joints2d[idx] if has_keypoints else None
        norm_img, raw_img, kp_2d = get_single_image_crop_demo(
            img,
            bbox,
            kp_2d=j2d,
